...
with st.sidebar:
    ...
    use_ml = st.toggle("≡ƒñû Predict f_ck via ML Model")
...
if element_choice == "RC Beams":
    ...
    with col2:
        ...
        <div class="metric-card">
            ...
            <div>Status: <span class="{status_color}">{status_text}: Capacity {cap_M:.1f} {'ΓëÑ' if cap_M >= M_eq else '<'} Applied {M_eq:.1f}</span></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if boq['total_cost'] > 40000:
            st.error("ΓÜá∩╕Å High Material Cost Alert! This beam design may be uneconomical. Immediate redesign is recommended.")
            ...
            st.warning("≡ƒÆí Consider optimizing the beam design to reduce material costs. Suggestions may include modifying width (b), depth (D), or tension steel (A_st).")
            st.write("Optimizing designs can lead to significant cost savings while ensuring safety and compliance with IS 456.")
        else:
            st.success("Γ£à Material costs are within an acceptable baseline range.")
        ...
        if is_safe:
            ...
            st.warning("ΓÜá∩╕Å **Status: Redesign Required** ΓÇö Costs below reflect the current (non-compliant) design and will change upon structural correction.")
            
        # Display the BOQ table cleanly
        st.table({
            "Parameter": ["Concrete Volume (m┬│)", "Steel Weight (kg)", "Total Material Cost (Γé╣)"],
            "Value": [round(boq['volume_m3'], 2), round(boq['steel_kg'], 2), round(boq['total_cost'], 2)]
        })
        
        st.write("### Detailed Breakdown of Costs:")
        st.write(f"- Concrete Volume: **{boq['volume_m3']} m┬│** at Γé╣ {boq['concrete_cost_per_m3']:,.2f} per m┬│")
        st.write(f"- Steel Weight: **{boq['steel_kg']} kg** at Γé╣ {boq['steel_cost_per_kg']:,.2f} per kg")
        cost_per_m = boq['total_cost'] / (L / 1000)
        st.write(f"- Total Cost per Meter Span: **Γé╣ {cost_per_m:,.2f} per m**")
        
        # 7. PDF Report
        pdf_bytes = create_pdf_report(verifier, boq, fig)
        st.download_button(label="≡ƒôä Download Official PDF Report", data=pdf_bytes, file_name="IS456_Report_Beam.pdf", mime="application/pdf", type="primary")
    ...
    with col2:
        ...
        if boq['total_cost'] > 50000:
            st.error("ΓÜá∩╕Å High Material Cost Alert! This column design may be uneconomical.")
            st.write("Consider alternative layouts or higher grade concrete (f_ck).")
        elif boq['total_cost'] > 25000:
            st.warning("≡ƒÆí Consider optimizing the column design to reduce material costs.")
        ...
        if "PASS" in pm_status and is_safe:
            ...
            st.warning("ΓÜá∩╕Å **Status: Verification Pending** ΓÇö Costs below reflect the preliminary design and may change if SP-16 charts demand a larger section.")
        else:
            st.write("While material volumes are low, this design **FAILS structural compliance**. Redesign required.")
            st.warning("ΓÜá∩╕Å **Status: Redesign Required** ΓÇö Costs below reflect the current (non-compliant) design and will change upon structural correction.")
        ...
        st.write(f"- Total Cost per Meter Height: **Γé╣ {cost_per_m:,.2f} per m**")
        
        # 5. PDF Report
        pdf_bytes = create_pdf_report(verifier, boq, fig)
        st.download_button(label="≡ƒôä Download Official PDF Report", data=pdf_bytes, file_name="IS456_Report_Column.pdf", mime="application/pdf", type="primary")
    ...
    with col2:
        ...
        if boq['total_cost'] > 120000:
            st.error("ΓÜá∩╕Å High Material Cost Alert! This slab design may be uneconomical. Immediate redesign is recommended.")
        elif boq['total_cost'] > 60000:
            st.warning("≡ƒÆí Consider optimizing the slab design to reduce material costs. Suggestions may include modifying overall thickness (D).")
        else:
            st.success("Γ£à Material costs are within an acceptable baseline range.")
        ...
        if is_safe:
            ...
            st.write("While material volumes are low, this design **FAILS structural compliance**. Redesign required.")
            st.warning("ΓÜá∩╕Å **Status: Redesign Required** ΓÇö Costs below reflect the current (non-compliant) design.")
        ...
        st.write(f"- Total Cost per Square Meter: **Γé╣ {cost_per_m2:,.2f} per m┬▓**")

        pdf_bytes = create_pdf_report(verifier, boq, fig)
        st.download_button(label="≡ƒôä Download Official PDF Report", data=pdf_bytes, file_name="IS456_Report_Slab.pdf", mime="application/pdf", type="primary")
    ...
    with col2:
        ...
        if boq['total_cost'] > 100000:
            st.error("ΓÜá∩╕Å High Material Cost Alert! This footing design may be uneconomical.")
        elif boq['total_cost'] > 50000:
            st.warning("≡ƒÆí Consider optimizing the footing design to reduce material costs.")
        else:
            st.success("Γ£à Material costs are within an acceptable baseline range.")
        ...
        st.write(f"- Concrete Volume: **{boq['volume_m3']} m┬│** at Γé╣ {boq['concrete_cost_per_m3']:,.2f} per m┬│")
        st.write(f"- Steel Weight: **{boq['steel_kg']} kg** at Γé╣ {boq['steel_cost_per_kg']:,.2f} per kg")
        st.write(f"- Total Material Cost: **Γé╣ {boq['total_cost']:,.2f}**")
        st.write(f"- **Total Cost per Unit Length: Γé╣ {boq['total_cost']/L:,.2f} per mm**")
        
        pdf_bytes = create_pdf_report(verifier, boq, fig)
        st.download_button(label="≡ƒôä Download Official PDF Report", data=pdf_bytes, file_name="IS456_Report_Footing.pdf", mime="application/pdf", type="primary")
    ...
    st.download_button(f"≡ƒôÑ Download {batch_type} Sample CSV", data=sample_csv, file_name=f"IS456_Batch_{batch_type.replace(' ', '_')}.csv", mime="text/csv")
    ...
    if uploaded_file is not None:
        try:
            ...
            if st.button(f"≡ƒÜÇ Run Bulk IS 456 Verification ({batch_type})", type="primary"):
                results = []
                ...
                st.success(f"Γ£à Successfully processed {len(df)} {batch_type.lower()} through the IS 456 engine!")
                ...
                st.download_button(
                    label=f"≡ƒôÑ Download Verified Dataset (CSV)",
                    data=csv_export,
    ...
    if uploaded_ifc is not None:
        ...
        try:
            ...
            st.success("Γ£à IFC Model loaded and parsed successfully!")
        ...
        if parse_success:
            if df_beams.empty and df_columns.empty and df_slabs.empty and df_footings.empty:
                st.warning("ΓÜá∩╕Å **No recognizable structural elements were found.**")
                ...
                with t_beam:
                    if not df_beams.empty:
                        ...
                        if st.button("≡ƒÜÇ Verify BIM Beams", type="primary"):
                            if (edited_beams['b'] <= 0).any() or (edited_beams['D'] <= 0).any():
                                st.error("ΓÜá∩╕Å Validation Error: All beams must have Width (b) and Depth (D) greater than 0.")
                            else:
                                results = []
                ...
                with t_col:
                    if not df_columns.empty:
                        ...
                        if st.button("≡ƒÜÇ Verify BIM Columns", type="primary"):
                            if (edited_cols['b'] <= 0).any() or (edited_cols['D'] <= 0).any():
                                st.error("ΓÜá∩╕Å Validation Error: All columns must have width (b) and depth (D) greater than 0.")
                            else:
                                results = []
                ...
                with t_slab:
                    if not df_slabs.empty:
                        ...
                        if st.button("≡ƒÜÇ Verify BIM Slabs", type="primary"):
                            if (edited_slabs['L_x'] <= 0).any() or (edited_slabs['L_y'] <= 0).any() or (edited_slabs['d'] <= 0).any():
                                st.error("ΓÜá∩╕Å Validation Error: You must replace all 0.0 spans (L_x, L_y) and depths (d) with actual values before verifying.")
                            else:
                                results = []
                ...
                with t_foot:
                    if not df_footings.empty:
                        ...
                        if st.button("≡ƒÜÇ Verify BIM Footings", type="primary"):
                            if (edited_footings['L'] <= 0).any() or (edited_footings['B'] <= 0).any() or (edited_footings['d'] <= 0).any():
                                st.error("ΓÜá∩╕Å Validation Error: You must replace all 0.0 dimensions (L, B, d) with actual values before verifying.")
                            else:
                                results = []
